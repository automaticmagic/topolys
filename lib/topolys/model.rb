...
module Topolys
  ...
  class Wire < Object
    ...
    def recalculate
      ...
      # Recourse to Float::EPSILON occurs in 4x places in model.rb. Direct
      # comparison between Float::EPSILON (i.e. 2.2204460492503131e-16)
      # and something like a distance (in millimeters) is problematic. If
      # epsilon were instead a relative tolerance (one could be more stringent
      # when assessing a 8cm x 8cm surface instead of a 8m x 8m surface), then
      # that could work. But for now, I think we're OK with the absolute 'tol'
      # of 10mm (some days I think this could be stretched to 25mm) for
      # vertex equivalence, is a vertex part of a line or a plane, etc.

      # If I replace all occurrences of Float::EPSILON with 0.01, all
      # raises/errors go away. I attempted to use @tol, but this raised
      # a 'comparison of Float with nil failed', which I think is related to
      # the Model initialize method. To re-assess?

      @directed_edges.each do |de|
        raise "Point not on plane - 2" if (de.v0.point - @plane.project(de.v0.point)).magnitude > 0.01 #Float::EPSILON
        if (de.v1.point - @plane.project(de.v1.point)).magnitude > 0.01  #Float::EPSILON
          puts "Point not on plane - 3" #raise
          puts "v0:#{de.v0.point.x},#{de.v0.point.y},#{de.v0.point.z}"
          puts "mag: #{(de.v1.point - @plane.project(de.v1.point)).magnitude}"
        end
      end
    end
  ...
  class Face < Object
    ...
    def recalculate
      ...
      normal = @outer.normal
      @holes.each do |hole|
        raise "Hole does not have correct winding" if hole.normal.dot(normal) > -1 + 0.01 # Float::EPSILON
      end
      ...
      @holes.each do |hole|
        hold.points.each do |point|
          raise "Point not on plane - 1" if (point - plane.project(point)).magnitude > 0.01 # Float::EPSILON
        end
      end
